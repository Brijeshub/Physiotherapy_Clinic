...
const ImageUpload = () => {
  ...
  const [user, setUser] = useState(null);
  const [isClient, setIsClient] = useState(false);
  const [images, setImages] = useState([]); // Initial state for images
  const [image, setImage] = useState(null);
  // Verify right user auth
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        ...
        // console.log("Loading images from localStorage:", savedImages);
        setImages(savedImages);  // 👈 Important: Update state so UI can render them
      }
    });
  
    // 🔁 Also fetch from localStorage initially if no auth happens
    const savedImages = JSON.parse(localStorage.getItem("uploadedImages")) || [];
  ...
  const fetchClientData = async (uid) => {
    try {
      ...
      if (docSnap.exists()) {
        setIsClient(true);
        setImages(docSnap.data().images || []);
        // Save the images in localStorage
        localStorage.setItem(
          "uploadedImages",
          JSON.stringify(docSnap.data().images || [])
        );
      } else {
        setIsClient(false);
        setImages([]);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      localStorage.setItem("uploadedImages", JSON.stringify(updatedImages)); // Store to localStorage
      alert("Image uploaded successfully!");
    } catch (error) {
      console.error("Error uploading image:", error);
      alert("Failed to upload image.");
  ...
  const handleDelete = async (image) => {
    ...
    try {
      ...
      setImages(updatedImages);
      localStorage.setItem("uploadedImages", JSON.stringify(updatedImages)); // Update localStorage after delete
      alert("Image deleted successfully!");
    } catch (error) {
      console.error("Error deleting image:", error);
      alert("Failed to delete image.");
